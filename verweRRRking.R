...
## get data
file.list <- list.files("output_lab_ngt/")
file.list <- file.list[grep("results.txt",file.list)]

## open one temp file
tmp <- fromJSON(file=paste0(c("output_lab_ngt/",file.list[1]),collapse = ""))
...
tmp <- fromJSON(file=paste0(c("output_lab_ngt/",file.list[j]),collapse = ""))
pheno.tmp <- NULL
for (i in 1:length(tmp$observations)){
